...
metadata_filt <- metadata %>% filter(age == "6mo")
design_filt <- metadata_filt %>% transmute(sampleID = sample_name, condition = age) %>% mutate(
  condition = paste0("age_",condition)
)
...
pdf(here("isoform_level_analysis/isoformSwitchAnalyzer/satuRn/test.pdf"))
pl
dev.off()

write.csv(iso_fraction_df, here("isoform_level_analysis/isoformSwitchAnalyzer/satuRn/BDNF_isoform_fractions.csv"))
```
...
write.csv(iso_fraction_df, here("isoform_level_analysis/isoformSwitchAnalyzer/satuRn/TrkB_isoform_fractions.csv"))
```
