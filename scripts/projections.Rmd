...
```{r}
projection_pc <- prcomp(proj_res)
sample_weights <- projection_pc$rotation %>% as.data.frame() %>% tibble::rownames_to_column(var = "sample_name") %>% left_join(metadata)

sample_umap <- uwot::umap(as.data.frame( projection_pc$rotation),
                          n_neighbors = 2) %>% as.data.frame()
rownames(sample_umap) <- rownames(projection_pc$rotation)
colnames(sample_umap) <- c("UMAP1","UMAP2")
sample_umap <- sample_umap %>% tibble::rownames_to_column(var = "sample_name") %>% left_join(., metadata) 
